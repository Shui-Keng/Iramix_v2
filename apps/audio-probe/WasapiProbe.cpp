...
constexpr std::uint32_t kSampleRate = kGraphSampleRate;
constexpr std::uint16_t kChannels = kGraphChannels;
...
[[nodiscard]] std::uint64_t processAudio(
    ...
    const auto started = Clock::now();
    {
        ...
        workload.renderBlock(frameCount, samplePosition);
        const auto planar = workload.planarView(frameCount).asConst();
