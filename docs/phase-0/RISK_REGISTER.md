...
| R-02 | Plugin crash or hang destabilizes audio | 4 | 5 | 20 | Bounded-deadline shared-memory bridge measured: host survives child crash and hang, degrading to counted silence. See `results/PLUGIN_PROCESS_ISOLATION_2026-07-28.md`. The isolation evidence is against a stand-in child; real VST3 plugins are hosted separately in `results/VST3_PLUGIN_INSTANTIATION_2026-07-29.md` and the two are not yet joined. That work added a third reason for isolation beyond crash and hang: a Pure Data based plugin writes its banner to **stdout**, which is the transport P0-007 uses for the Java/C++ boundary, so an in-process plugin would corrupt the protocol rather than crash it |
| R-03 | Skiko/Skia GPU behavior differs substantially by OS | 4 | 4 | 16 | Run one renderer and device-loss spike per OS. **Still open — no GPU code has been written.** The raster slice in `results/SKIA_RASTER_BASELINE_2026-07-29.md` covers only `Surface.makeRasterN32Premul`: no `DirectContext`, no swap chain, no device-loss path. It does establish the harness the GPU spike will reuse (a deterministic scene, per-target screenshot baselines, and a frame-time counter line), and it measured the reason the GPU spike matters: CPU full-repaint costs 6.0–12.0 ms p50 on the reference machine and misses 120 Hz from 150% scale upward |
...
| R-11 | Cross-platform behavior drifts | 4 | 4 | 16 | Three-OS CI plus screenshot and project round-trip tests. Screenshot comparison now exists and runs in `gradle check` on all three CI operating systems, but baselines are committed for `windows-x64` only; macOS and Linux report `baseline=absent` and print a digest. **Whether Skia CPU raster is bit-identical across operating systems is unmeasured**, and the digests in `results/SKIA_RASTER_BASELINE_2026-07-29.md` are what a later comparison against CI logs would use to answer it |
