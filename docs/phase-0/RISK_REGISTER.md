...
| R-03 | Skiko/Skia GPU behavior differs substantially by OS | 4 | 4 | 16 | Run one renderer and device-loss spike per OS. **Partially open — slice 1 landed on Windows only.** `results/SKIA_GPU_BACKEND_FRAMETIME_2026-07-29.md` drives the Week 3 reference scene through a real `SkiaLayer` window: `getRenderApi()` reports Direct3D selected, 200 scheduled frames render without crashing, p50 frame time sits at the 60 Hz budget line (16.6ms) and misses 120 Hz at the median, and the tail widens materially under concurrent CPU load (p99 28ms isolated vs 67ms under `gradle check`). macOS and Linux backend selection remain unmeasured — no `MetalRedrawer` or `LinuxOpenGLRedrawer` path has run, and Linux hosted CI has no display by default, so whether this task can even execute there is still open. Resize, monitor movement, sleep/wake, and device-loss recovery are unstarted; only backend identification and a first frame-time trace are done |
