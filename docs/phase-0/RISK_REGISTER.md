...
| R-03 | Skiko/Skia GPU behavior differs substantially by OS | 4 | 4 | 16 | Run one renderer and device-loss spike per OS. **Partially open — slice 1 landed a backend matrix across all three CI OSes.** `results/SKIA_GPU_BACKEND_FRAMETIME_2026-07-29.md` drives the Week 3 reference scene through a real `SkiaLayer` window: `getRenderApi()` reports Direct3D on Windows, Metal on macOS, and Skiko's own software fallback on Ubuntu CI (an Xvfb step was added since hosted Linux has no display or GPU) — 200 scheduled frames render without crashing on every leg. On the Windows reference machine, p50 frame time sits at the 60 Hz budget line (16.6ms) and misses 120 Hz at the median, and the tail widens materially under concurrent CPU load (p99 28ms isolated vs 67ms under `gradle check`); CI timings are recorded but not compared, since hosted-runner hardware is uncontrolled and the Linux figure is a different (software) path entirely. A real Linux GPU backend (`LinuxOpenGLRedrawer` against actual hardware) is still unmeasured — only Ubuntu CI's no-GPU fallback has run. Resize, monitor movement, sleep/wake, and device-loss recovery are unstarted; only backend identification and a first frame-time trace are done |
