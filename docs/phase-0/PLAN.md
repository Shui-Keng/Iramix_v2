...
- Simulate resize, monitor movement, sleep/wake, and device loss —
  ...
  resumes completed Direct3D presents after every replacement. This is
  explicitly `CONTEXT_RECREATE_PROXY`; literal OS sleep/wake is an
...
- renderer recovery test — partial: resize recovery is measured on the
  ...
  measured locally as a sleep/wake proxy, literal OS sleep/wake is an
  accepted gap, and device loss remains;
