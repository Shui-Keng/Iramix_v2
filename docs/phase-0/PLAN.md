...
- two-hour callback soak-test results — done on Windows at 128 and 256
  frames
  ([`results/AUDIO_CALLBACK_SOAK_WINDOWS_2026-07-29.md`](results/AUDIO_CALLBACK_SOAK_WINDOWS_2026-07-29.md)):
  2,400 seconds per configuration, 1,349,419 callbacks, zero target
  misses, zero hard deadline misses, zero allocations, zero blocking
  locks. 64 frames was not soaked because no available device delivers
  the period (R-15), and macOS/Linux soaks remain blocked by R-13;
...
- renderer recovery test — partial: resize recovery is measured on the
  ...
  an accepted gap, and device loss remains;
