...
| P0-012 | Disk and session resilience | Native session ownership, write-ahead journal, monotonic undo/redo, replay, immutable/coalesced saves, fixed-window autosave, shutdown flush, revision-gated history compaction, and revisioned backup rotation/retention verified locally and on three-OS CI/Java/sanitizers; fail-closed automatic restore and durable journal baselines verified locally, hosted confirmation pending; complete state, cold hardware benchmarks, and media/plugin restoration pending |
