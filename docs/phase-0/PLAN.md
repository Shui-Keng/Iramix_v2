...
| P0-007 | Java–C++ process boundary | Complete for Phase 0 stdio transport; persistent handshake/load smoke green on three-OS CI. Load task now has a recorded result: 1,000 sequential pings against a live engine child, under a concurrent Skia raster load, measured at p50=0.061–0.127 ms across the Windows reference machine and all three CI operating systems in Debug builds (see [`results/IPC_LOAD_UNDER_UI_LOAD_2026-07-29.md`](results/IPC_LOAD_UNDER_UI_LOAD_2026-07-29.md)). The task's `uiFrames` counter was racy on macOS and is now a bounded liveness signal only, explicitly not a frame-rate or responsiveness figure. Investigating that flake produced the more consequential finding: the concurrent Skia render thread is **starved** by saturated IPC traffic, its frame period rising from a 1.4–1.9 ms idle baseline to 3.1 ms on Ubuntu, 11.2 ms on Windows, and 55.8 ms on macOS — 1.6×/8.1×/29.6×, which is why only the macOS leg ever reached zero frames. The cause of the per-OS spread is not established, the factors are quantized by frame counts as low as three, and 1,000 back-to-back blocking pings is a deliberately pathological pattern rather than a model of real UI traffic. Whether a paced or batched command stream starves the renderer at all is open and should be answered before Phase 1 commits to this transport shape. Optimized-build latency, non-ping payloads, concurrent audio-callback load, and physical macOS/Linux hardware remain untested |
