...
| P0-012 | Disk and session resilience | Native session ownership, write-ahead journal, monotonic undo/redo, replay, immutable/coalesced saves, fixed-window autosave, shutdown flush, and revision-gated history compaction verified locally and on three-OS CI/Java/sanitizers; revisioned backup rotation/retention and failure isolation verified locally, hosted confirmation pending; automatic restore, complete state, cold hardware benchmarks, and media/plugin restoration pending |
