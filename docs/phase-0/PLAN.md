...
- Simulate resize, monitor movement, sleep/wake, and device loss —
  resize slice done on the Windows Direct3D reference machine
  ...
  with 45 backend presents and no crash, exception, timeout, or hang.
  Monitor movement is **not done**: this machine exposes one
  `GraphicsDevice`, so the task reports
  `LIMITATION_SINGLE_MONITOR` rather than simulating a same-device move.
  Sleep/wake and device loss remain unstarted.
...
- renderer recovery test — partial: resize recovery is measured on
  Windows Direct3D; monitor-move is blocked by single-monitor hardware,
  and sleep/wake/device-loss slices remain;
