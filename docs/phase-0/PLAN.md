...
| P0-012 | Disk and session resilience | Native session ownership, write-ahead journal, monotonic undo/redo, replay, immutable/coalesced saves, fixed-window autosave, shutdown flush, and revision-gated history compaction implemented; prior slice is green on three-OS CI/Java/sanitizers, local MSVC/GCC tests pass for this slice, hosted verification pending; backup rotation, complete state, cold hardware benchmarks, and media/plugin restoration pending |
