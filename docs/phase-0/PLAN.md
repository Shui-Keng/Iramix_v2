...
accepted as unachievable (R-16). See [`EXIT_REVIEW.md`](EXIT_REVIEW.md)
(2026-07-29) for what remains and why none of it is an engineering
blocker. The team is one developer plus an AI assistant, recorded
2026-07-29; R-01 is evaluable but still awaiting a scope decision.  
Started: 2026-07-27
...
- ~~Review the product brief with at least three composers and three sound
  designers.~~ **Unachievable; accepted as R-16 on 2026-07-29** — no
  target users are reachable to this project.
- ~~Rank the top ten workflow problems.~~ Depends on the above; no
  user-derived ranking exists.
...
- ~~interview notes and ranked problems~~ — unachievable, accepted as
  R-16;
- approved v1 scope — a draft exists and is **approved but unvalidated**;
  it has also not been re-cut against the recorded team size of one
  developer (R-01);
- green skeleton build on three CI operating systems — done.
...
- Review research findings with target users — **unachievable, accepted
  as R-16** (P0-009 closed on 2026-07-29).
- Convert validated spikes into a Phase 1 backlog — done, with
  structural sizing and no owners.
- Decide whether architecture ADR-0003 is accepted — **decided
  2026-07-29: accepted for Windows Tier-1**, carrying its two recorded
  qualifications (fixture-level PDC after routing changes, unmeasured
...
| P0-009 | User interviews | **Closed as unachievable on 2026-07-29, accepted as risk R-16.** The project has no reachable composers or sound designers, so this cannot be performed rather than merely not yet performed — the same treatment R-13 and R-15 receive. Consequence recorded rather than softened: the product brief and v1 scope stay hypotheses, no user-derived ranking of workflow problems exists, and R-01's response ("enforce the v1 scope contract") is left bounding effort without aiming it. Interaction prototypes are explicitly *not* blocked by this — only testing them with users is — so they remain in the Phase 1 backlog as P1-A2 |
| P0-010 | Dependency/license inventory | Inventory separates in-build from candidates, Java licenses read from cached POMs and regenerable via `scripts/license-inventory.ps1`; C++ engine confirmed dependency-free. Four obligations open: Skiko native attribution (L-1), unverified JACK license (L-2), mutable CI action tags (L-3), annotations version skew (L-4); vulnerability inventory still unautomated. **Updated 2026-07-29: L-3 closed**, all four CI `uses:` entries pinned to full commit SHAs with version comments. **L-2 closed** — measured on the Ubuntu runner rather than assumed from upstream reputation: the client library is LGPL-2.1+ under the blanket `Files: *` stanza (the GPL entries govern one server file and the `zalsa` tools, neither linked here), the packages ship `libjack.so`/`libjack.so.0` and **no `.a`**, so static linking is impossible rather than merely discouraged, and `pkg-config --libs jack` emits a plain `-ljack`. Dynamic linkage against LGPL-2.1+ satisfies the no-GPL-in-a-closed-source-path policy. **L-1 partially discharged and found to be bigger than recorded**: inspecting the pinned artifact rather than reasoning about it turned up compiled-in copyright notices for libpng, libjpeg-turbo, and zlib, a version marker for Expat 2.7.4, and name-level references to HarfBuzz, libwebp, Wuffs, and ANGLE — against an inventory that had assumed Skia and ICU alone. [`NOTICE.md`](../../NOTICE.md) carries the verbatim attribution strings with an evidence grade per component, and states plainly that it is a floor rather than a discharge: only `windows-x64` was inspected, string scanning cannot prove absence, and no upstream manifest was obtained. L-4 untouched |
